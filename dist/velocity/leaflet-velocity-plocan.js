...
L.CanvasLayer = (L.Layer ? L.Layer : L.Class).extend({
	...
	onAdd: function onAdd(map) {
		...
		/////////////////////////////////////	
		//TODO: para agnadir a un pane si se define
		if (this._delegate.options.pane){
		   map._panes[this._delegate.options.pane].appendChild(this._canvas);
		} else{
	...
	onRemove: function onRemove(map) {
		...
		//TODO: se elimina del pane definido en options
                if (this._delegate.options.pane){
                   map._panes[this._delegate.options.pane].removeChild(this._canvas);
                } else{
...
//TODO:
var velocitiesArray = new Array(); //hay que ver como gestionar este array de otra manera, no como variable global. Es inconsistente
//TODO: y capturar el canvas directamente en vez de las VelocityLayers?
...
L.Control.Velocity = L.Control.extend({
	...
	vectorToSpeed: function vectorToSpeed(uMs, vMs, unit, conversion) { //TODO: se agnade conversion
		...
		} else if (unit === 'cm/s') { //TODO: 
		    return this.meterSec2cmSec(velocityAbs); 
		} else {
			return velocityAbs;
		}
		*/
		if (conversion != false){ //TODO:
		    if (unit === 'k/h') {
			    return this.meterSec2kilometerHour(velocityAbs);
		    } else if (unit === 'kt') {
			    return this.meterSec2Knots(velocityAbs);
	...
	meterSec2cmSec: function meterSec2cmSec(meters) { //TODO: 
		return meters * 100;
	},
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
			...
			htmlOut = "<strong>" + this.options.velocityType + " Direction: </strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(3) + "°" + ", <strong>" + this.options.velocityType + " Speed: </strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit, this.options.unitConversion).toFixed(1) + this.options.speedUnit;
			////////////////////////////////////
		} else {
...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
	...
	_startWindy: function _startWindy() {
		...
		this._context.clearRect(0, 0, 3000, 3000); //TODO:
	...
	_initWindy: function _initWindy(self) {
		...
		this._windy = new Windy(options); //TODO: creacion de Windy!
		...
		this._map.on('dragstart', self._windy.stop); //TODO: EVENTOS!
	...
	_clearAndRestart: function _clearAndRestart() {
        velocitiesArray.forEach(function(o){ //TODO:
            if (o._context) o._context.clearRect(0, 0, 3000, 3000);
            //o._startWindy(); //TODO: sin start? parece funcionar mejor! 
        })
...
var Windy = function Windy(params) {
    ...
    //TODO:
    var CLICK = params.click;  //cambiar nombre variable
    var INV_AREA = params.inv_area || 40;
    var MODE = params.mode || "NORMAL"; //MODE could be "HIT" or "NORMAL"
    var FADE = (params.fade === undefined) ? true : params.fade;
    var AGING = (params.aging === undefined) ? true : params.aging;
    var MIN_MAGNITUDE = params.minMagnitude || 0;
    var N_WAVES = params.n_waves || 4;
    
    VELOCITY_SCALE *= (params.data[0].header.parameterUnit == 'cm/s') ? 1/100 : 1;
...
/////////////////////////////////////
	...
	var createWindBuilder = function createWindBuilder(uComp, vComp) {
	    ////////////////////////////////
	    //TODO: cambio de signo
	    //console.log("En createWindBuilder");
	    if (uComp.header.sign == -1){
	        var uData = uComp.data.map(function(element){return -element});
	    } else {
	        var uData = uComp.data;
        }
        if (vComp.header.sign == -1){
	        var vData = vComp.data.map(function(element){return -element});
	    } else {
  ...
  */
   ...
   */
		...
		field.randomize = function (o) { //TODO:
			...
			if (MODE === "HIT"){
			    ...
			    do {
				    ...
				    x = Math.round(GLOBX + Math.random()*bounds.width/INV_AREA - bounds.width/INV_AREA/2);
				    y = Math.round(GLOBY + Math.random()*bounds.height/INV_AREA - bounds.height/INV_AREA/2);
	...
	var click2xy = function click2xy(extent, bounds){ //TODO:
	    ...
	    if (CLICK){
	        ...
	        x_center = (CLICK.lng - rad2deg(extent.west))*extent.width / rad2deg(mapLonDelta);
	       
            var latrad = CLICK.lat*Math.PI/180;
            var a = Math.log(Math.tan( (latrad + Math.PI/2)/2 ));
            y_center = equatorY - a*worldMapRadius;
           
            return [x_center, y_center]; // sin Math.round
	...
	var animate = function animate(bounds, field) {
		...
		if (MODE != "HIT"){ //TODO: CREAR NUEVO MODE.
			for (var i = 0; i < particleCount; i++){	
				particles.push(field.randomize({ age: Math.floor(Math.random() * MAX_PARTICLE_AGE) + 0 }));
			}
		} else { //TODO: CREAR NUEVO MODE. OLEADA?
			for (var j = 0; j < N_WAVES; j++){
				for (var i = 0; i < Math.floor(particleCount/N_WAVES); i++) {
					particles.push(field.randomize({age : Math.floor(MAX_PARTICLE_AGE * (j + 1) /   N_WAVES)})); //TODO: a cascoporro
				}
			}
		}
		...
		function evolve() {
			...
			particles.forEach(function (particle) {
			    if (!AGING && particle.age >= MAX_PARTICLE_AGE){ //TODO:
			        deadParticles++;
			        //console.log(deadParticles + "/" + particleCount);
			        if (deadParticles == particleCount){stop();}
			        return;
			    }
			    
				else if (particle.age > MAX_PARTICLE_AGE) {
				...
				//if (m === null) { //TODO:
				if (m === null || m < MIN_MAGNITUDE) {
					if (MODE != "HIT"){ //TODO: ponerlo como and en la condicion anterior //MODO "OLEADAS" ?
				...
				if (AGING){particle.age += 1;}//TODO: las particulas no envejecen si AGING == false
			});
		}
		...
		function draw() {
			...
			if (FADE){	//TODO:
			    ...
			    g.globalCompositeOperation = "lighter"; //prev;
			    g.globalAlpha = 0.9;
			} else {
	...
	var start = function start(bounds, width, height, extent) {
		...
		[GLOBX, GLOBY] = click2xy(mapBounds, buildBounds(bounds, width, height)); //TODO:
